...
class PlaylistManager {
    ...
    async create(title, video_ids) {
        ...
        const response = await __classPrivateFieldGet(this, _PlaylistManager_actions, "f").execute(CreateEndpoint.PATH, CreateEndpoint.build({
            ids: video_ids,
            title
        }));
        return {
    ...
    async delete(playlist_id) {
        ...
        if (!__classPrivateFieldGet(this, _PlaylistManager_actions, "f").session.logged_in)
            throw new InnertubeError('You must be signed in to perform this operation.');
        const response = await __classPrivateFieldGet(this, _PlaylistManager_actions, "f").execute(DeleteEndpoint.PATH, DeleteEndpoint.build({
            playlist_id
        }));
        return {
            playlist_id,
    ...
    async addVideos(playlist_id, video_ids) {
        ...
        if (!__classPrivateFieldGet(this, _PlaylistManager_actions, "f").session.logged_in)
            throw new InnertubeError('You must be signed in to perform this operation.');
        const response = await __classPrivateFieldGet(this, _PlaylistManager_actions, "f").execute(EditPlaylistEndpoint.PATH, EditPlaylistEndpoint.build({
    ...
    async removeVideos(playlist_id, video_ids, use_set_video_ids = false) {
        ...
        if (!__classPrivateFieldGet(this, _PlaylistManager_actions, "f").session.logged_in)
            throw new InnertubeError('You must be signed in to perform this operation.');
        const info = await __classPrivateFieldGet(this, _PlaylistManager_actions, "f").execute(BrowseEndpoint.PATH, { ...BrowseEndpoint.build({ browse_id: `VL${playlist_id}` }), parse: true });
        ...
        const response = await __classPrivateFieldGet(this, _PlaylistManager_actions, "f").execute(EditPlaylistEndpoint.PATH, EditPlaylistEndpoint.build(payload));
        return {
            playlist_id,
            action_result: response.data.actions // TODO: implement actions in the parser
        };

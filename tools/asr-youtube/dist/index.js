import { getVideoMeta, extractAudioTemp } from './media/youtube.js';
import { transcribeAudio } from './openai/whisper.js';
import { reconcileChapters } from './chapters/reconcile.js';
import { paragraphizeSegments } from './text/paragraphize.js';
import { writeCanonicalJSON } from './output/canonical.js';
import { writeVTT } from './output/vtt.js';
import { writeSRT } from './output/srt.js';
import { log, progress, success, error } from './util/log.js';
export async function transcribeYouTubeVideo(options) {
    const { url, lang = 'en', paragraphMin = 2, paragraphMax = 4, out = './out', title, rights = 'owner_ok', vtt = false, srt = false, asrModel = 'whisper-1', useDedalus = false } = options;
    try {
        // Extract video ID
        ...
        try {
            // Step 4: Transcribe with Whisper (via Dedalus or direct)
            progress(`Transcribing with ${useDedalus ? 'Dedalus-routed' : 'direct OpenAI'} Whisper...`);
...
function assignSegmentsToChapters(segments, chapters) {
    return segments.map(segment => {
        ...
        const chapter = chapters.find(ch => segment.start_s >= ch.start_s && segment.start_s < ch.end_s);
        return {
            ...segment,
